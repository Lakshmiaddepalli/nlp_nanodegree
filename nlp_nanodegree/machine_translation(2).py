...
def encdec_model(input_shape, output_sequence_length, english_vocab_size, french_vocab_size):
    ...
    print(input_shape[1:])
    a = Input(input_shape[1:])
    layer = LSTM(output_sequence_length)(a)
    layer = RepeatVector(output_sequence_length)(layer)
    layer = LSTM(output_sequence_length,return_sequences=True)(layer)
    predictions = TimeDistributed(Dense(french_vocab_size, activation='softmax'))(layer)
    model = Model(inputs=a, outputs=predictions)
    learning_rate = 0.005
    model.compile(loss=sparse_categorical_crossentropy,
...
score = simple_encdec_model.evaluate(tmp_x, preproc_french_sentences, verbose=0)
print("Train accurancy: ", score[1])
